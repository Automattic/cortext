...
### The `cortext` inserter category is ready for the full inserter

`src/components/cortextBlockCategory.js` registers a `cortext` category at module load, before the `../blocks` barrel runs. Without that, Gutenberg drops `"category": "cortext"` while it processes each block registration.

Categories show up in the full inserter panel, not in the Quick Inserter that Cortext currently exposes. This wiring means Cortext-native blocks are already grouped when the Quick Inserter's "Browse All" button starts opening the full inserter (GitHub issue #85 sub-item).
