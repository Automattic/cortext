...
-   `crtxt_document` stores every editable record. A document is a page, a collection, or a row depending on its state: a `cortext_fields` schema designates it a collection (which mirrors itself as a `crtxt_trait` term), a `crtxt_trait` taxonomy term makes it a row of that collection, and neither makes it a plain page.
