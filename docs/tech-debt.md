...
-   A menu inside `BlockCanvas` cannot appear above Gutenberg's contextual toolbar because the menu and toolbar are in separate documents, so their `z-index` values are unrelated. `CanvasMenuToolbarGuard` maps the menu's bounds to the parent viewport and hides that canvas's toolbar only when the two overlap. It depends on Ariakit's `[data-dialog][role="menu"][data-open]` contract and Gutenberg's `.block-editor-block-list__block-popover` class.
...
**Where.** `src/components/fields/FieldActionsMenu.js` (outside-click document listener, portal opt-in, hover bridge), `src/components/fields/ColumnHeaderActions.js` and `src/components/fields/ColumnHeaderActions.scss` (destructive item class and rule), `FieldFormatPopover` and `TableCalculationMenu` (sibling submenus and outside-click guards), `useSubmenuPlacement` in `src/hooks/useSubmenuPlacement.js`, `EditOptionsPopover` in `src/components/fields/EditOptionsPopover.js` (nested popover dismiss), and `CanvasMenuToolbarGuard` in `src/components/EditorBody.js` plus `src/styles/global/_shell-root.scss` (hiding the toolbar when it overlaps a menu from another document).

**Solution.** These fixes belong upstream in Gutenberg's `@wordpress/components` package. For this bug, `Menu.Popover` needs a portal target in the parent document. Once the menu and toolbar share a document, regular `z-index` ordering works. The other gaps need separate Gutenberg PRs: outside-click handling across documents, a destructive menu-item variant, portalling by default, arbitrary submenu content, coordinated dismissal for nested popovers, and configurable placement fallbacks.
...
**WorDBless can't validate the rows endpoint's SQL paths.**

**What.** WorDBless uses `Db_Less_Wpdb`, an in-memory store. Calls to `wp_insert_post` and `get_post` work through the object cache, but `WP_Query` SQL returns no results. `InMemoryPostsQuery` handles the simple query shapes used by document listing, page-trash cascades, and row reads: post type, status, parent, taxonomy membership, `meta_key` / `meta_value`, search, ordering, projection, and pagination. `InMemoryTermStore` stores the term relationships those queries need. These shims let `RowsController` tests insert rows and retrieve them through real `GET /cortext/v1/rows` requests, with both full and ID-only response shapes.

WorDBless still cannot run the SQL that Cortext assembles outside `WP_Query`'s normal arguments. That includes the custom joins, grouped filters, comparisons, search order, and indexed filter/sort reads built by `RowsFilterQuery`, `RowsMetaQuery`, `RowsQueryScope`, and the field-value index. The readable-index test can only confirm that an ID-only request selected the index branch. Its empty result does not show whether the returned IDs, order, totals, or pagination are correct. A bad join, `meta_query` compare, title sentinel, index predicate, or projected row response could still pass here.

**Where.** `tests/php/InMemoryPostsQuery.php`, `tests/php/InMemoryTermStore.php`, `tests/php/test-in-memory-posts-query.php`, `tests/php/test-in-memory-term-store.php`, `tests/php/test-rest-rows-controller.php`, `tests/php/test-field-value-read-query.php`, and the document/cascade suites that use the same shims.
