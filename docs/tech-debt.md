...
Drag/drop and `menu_order` accounting now look at both pages and collections through `treeRecords`, but the wiring is still uneven. Nested collection rows render inside the Pages `DndContext`; top-level collection rows render in the separate Collections section, outside that context, even though `CollectionRow` registers draggable/droppable zones. The cycle guard is page-only too: it asks whether the drop target id is a descendant page, so dropping a page before/after a collection whose parent is a descendant can still produce a bad reparenting target. Lazy loading lives in RSM-1848; this entry is about the missing shared model and the local drag/drop edge cases that fall out of it.

**Where.** `ACTIVE_PAGES_QUERY` in `src/components/page-queries.js`, `FULL_PAGE_COLLECTION_QUERY` in `src/collections.js`, `nestedCollections` / `topLevelCollections`, `treeRecords`, `handleDragOver`, and the Pages-only `DndContext` in `src/components/Sidebar.js`, the `renderCollectionRow` bridge in `src/components/PageRow.js`, `CollectionRow`'s drag/drop zones, `computeDropTarget` in `src/components/pages-tree.js`, and the `Collection::hierarchical` / `CollectionsController::validate_parent_document` setup on the PHP side.

**Solution.** Add a workspace-tree REST endpoint that returns navigable nodes with `kind`, `id`, `parent`, `menu_order`, and visibility in one shape. Row-owned collections could then appear under their row, missing parents would not look top-level by accident, and page/collection branching would move out of every consumer. Until then, top-level collections need to live under the same drag/drop context as pages, and cycle checks need to run against the merged node graph instead of page records only.
