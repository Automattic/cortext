...
The dynamic `crtxt_{slug}` post types already use `show_in_rest`, so `core-data` should be able to discover them lazily. Rows just do not use that path yet. Writes still go straight through `apiFetch`, then ask the hook to refetch.
...
The rows endpoint is the part we still cannot test cleanly. `RowsController` unit tests cover routing, permissions, validation, query-arg building, and row formatting. The last two still go through reflection because WorDBless cannot insert rows and then prove that `GET /cortext/v1/rows` returns them. `RowsFilterQuery` and `RowsMetaQuery` build SQL for custom compares and grouped filters; a bad join, `meta_query` compare, or title sentinel could still pass these tests.
...
**What.** Relation fields store row post IDs, but the UI behaves like a reference field: search rows in another collection, pick one or many, create a missing row, show row chips, and open the row from the chip. DataViews has no `relation` / `reference` field type and DataForm has no async record picker that accepts a target entity/query and cardinality. Cortext maps relations to the closest DataViews metadata type, keeps relation metadata on the field object, renders chips from `field.render`, uses a `useCollectionRows` picker, and handles chip clicks in its own peek state. The picker, chip rendering, and open action are still Cortext code.
...
**Solution.** Upstream DataViews/DataForm (or shared WP components) could expose a reference field/control with target entity config, single vs multi cardinality, async search, optional create-new UI, token/chip rendering hooks, and a supported action area for opening referenced records. Cortext would still own the backend relation sync and reverse-field semantics, but could drop most of the custom picker, chip, and open-action code. It also would not need to carry relation metadata through DataViews field objects.
