...
Knowledge bases need documents, typed records, views, and relations. WordPress has the storage pieces for that work, but the editing experience is scattered across custom fields, list tables, plugin screens, and theme templates.
...
WordPress core now has enough of the pieces to make this practical: posts, post meta, REST, the block editor, and DataViews. Cortext wires those together into one knowledge base surface.

## Design principles

### 1. WordPress-native first

Every architectural decision should start with WordPress primitives. No custom tables unless the post/meta model clearly fails. No REST API bypass. No fighting the block editor.
